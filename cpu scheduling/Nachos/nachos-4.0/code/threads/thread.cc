...
//----------------------------------------------------------------------
// simulateThread
//  Simulate time running and the burst time of the thread will decrease.
//----------------------------------------------------------------------
...
void simulateThread(){
    ...
    while (thread -> getburstTime() > 0){
        ...
        kernel -> interrupt -> OneTick(); // advance simulated time and if there are any pending interrupts to be called
    }
}

//-----------------------------------------------------------------------
// schedulingTest
//  Design a test code and initialize it. Set up a ping-pong between two 
//  two threads, by forking a thread to call the function simulateThread(),
//  and then calling the function simulateThread().
//------------------------------------------------------------------------
void Thread::SchedulingTest(){
    // create threads and their parameters
    ...
    kernel -> currentThread -> Yield(); // Relinquish the CPU if any other thread is ready to run. If so, put the thread at the end of the ready list, so that it will eventually be re-scheduled.
                                        // It will return if no other thread in the ready queue.
}
