...
bool AddrSpace::UsedPageState[NumPhysPages] = {0};
...
{   
    for (unsigned int i = 0; i < numPages; i++){
        AddrSpace::UsedPageState[pageTable[i].physicalPage] = false;
    }
...
// how big is address space?
    ...
    for (unsigned int i = 0, j = 0; i < numPages; i++) {
	    pageTable[i].virtualPage = i;	// for now, virt page # = phys page #
        while (j < NumPhysPages && AddrSpace::UsedPageState[j] == true){
            j++;
        }
            AddrSpace::UsedPageState[j] = true;
